...
# |gamma| threshold (kJ/mol) below which _alpha_tensor returns a graph-
# connected zero to prevent d(alpha)/d(gamma) from diverging.  Distinct from
# TorchAFIRTerm.gamma_min_abs (default 0.1), which is an *initialisation*
# warning; this constant is a *runtime* safety guard active during learning.
_ALPHA_GUARD_THRESHOLD: float = 1e-6
...
def _alpha_tensor(gamma_kjmol: torch.Tensor) -> torch.Tensor:
    ...
    Notes
    -----
    The smooth approximation ``g_smooth = sqrt(g² + ε²)`` uses ``ε = 1e-12``
    (i.e. ``ε² = 1e-24``) to ensure that ``alpha(g)`` is C¹-continuous at
    ``g = 0`` (no kink).  Note that the literal ``1e-24`` in the code represents
    ``ε²``, so the effective smoothing scale is ``ε = 1e-12``.
    However, the gradient magnitude ``|∂alpha/∂g|`` peaks at ``~1/(C·ε)``
    ...
    Near-zero guard (B4 / torch.compile compatibility):
    When ``|gamma_kjmol| < 1e-6`` the denominator in the alpha formula
    collapses toward zero (denom ~ g_smooth ~ 1e-12 a.u.), causing
    ``∂alpha/∂gamma`` to diverge and blow up any gradient-based optimizer.
    ...
    # sqrt(g² + eps²) so that d(alpha)/d(gamma) is continuous everywhere.
    # ε = 1e-12, so ε² = 1e-24.
    ...
    return torch.where(gamma_kjmol.abs() < _ALPHA_GUARD_THRESHOLD, gamma_kjmol * 0, alpha_normal)
