...
class BiasTerm(ABC):
    ...
    * **Optional**: override ``supports_autograd`` to return ``True`` and
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        :exc:`AttributeError` later in ``BiasCalculator.__init__()``.

        No-``__init__`` subclasses are handled by :meth:`BiasTerm.__init__`
        acting as an MRO fallback (see the Bug 1 fix comment there).

        Dataclass-generated ``__init__`` methods always set ``self.name``
        (``name: str`` is a required field), so they do not need wrapping.
        The ``@dataclass`` decorator fires *after* this hook, so its generated
        ``__init__`` is not in ``cls.__dict__`` at this point and is unaffected.
        """
